...
# Return an array with the 8 surrounding cells, plus some to easy our life afterwards (if they don't exist, return Null in this space instead)
# Cells are returned in the clockwise order starting from the top
def look_around(row, column):
    global input_array
    global row_limit
    global column_limit
    neighbors = [None] * 9
    ...
    if row > 0:
        neighbors[0] = input_array[row - 1][column]
        neighbors[-1] = input_array[row - 1][column]
    if row > 0 and column < column_limit - 1:
        neighbors[1] = input_array[row - 1][column + 1]
    if column < column_limit - 1:
        neighbors[2] = input_array[row][column + 1]
    if row < row_limit - 1 and column < column_limit - 1:
        neighbors[3] = input_array[row + 1][column + 1]
    if row < row_limit - 1:
        neighbors[4] = input_array[row + 1][column]
    if row < row_limit - 1 and column > 0:
        neighbors[5] = input_array[row + 1][column - 1]
    if column > 0:
        neighbors[6] = input_array[row][column - 1]
    if row > 0 and column > 0:
        neighbors[7] = input_array[row - 1][column - 1]
    return neighbors
...
def build_indexed_array(row = 0, column = 0):
    ...
    around = look_around(row, column)
    corners = 0
    for i in [0, 2, 4, 6]:
        symbol_at_cell1 = around[i]
        symbol_at_cell2 = around[i+1]
        symbol_at_cell3 = around[i+2]
        if symbol_at_cell1 != symbol and symbol_at_cell3 != symbol:
            corners += 1
        if symbol_at_cell1 == symbol and symbol_at_cell2 != symbol and symbol_at_cell3 == symbol:
            corners += 1
    corners_by_index[index] = corners_by_index.get(index, 0) + corners
...
with open('input.txt', 'r') as file:
    ...
    for i in range(index):
        print_verbose(f"index: {i} symbol: {symbol_by_index[i]} area: {area_by_index[i]} corners: {corners_by_index[i]}")
        result += area_by_index[i] * corners_by_index[i]
